...
namespace GB28181.Client
{
    public partial class GB28181SipClient<T> where T : ChannelItem
    {
        ...
        private const int DEFAULT_REGISTER_FAILURE_RETRY_INTERVAL = 60;
        private const int DEFAULT_MAX_REGISTER_ATTEMPTS = 3;

        //private static ILogger logger = SQ.Base.Log.Log4;
        ...
        /// <param name="server_id">The server_id to use if the server requests authorisation.</param>
            ...
            bool exitOnUnequivocalFailure = true)
        {
            ...
            if (SIPURI.TryParse(server, out var serverUri))
            {
                ...
                m_sipAccountAOR.User = server_id;
            }
            else
            {
                m_sipAccountAOR = new SIPURI(server_id, server, null, SIPSchemesEnum.sip, SIPProtocolsEnum.udp);
            }

            m_authUsername = server_id;
        ...
        private void DoRegistration(object state)
        {
            if (Monitor.TryEnter(m_waitForRegistrationMRE))
            {
                ...
                {
                    SQ.Base.ErrorLog.WriteLog4Ex("DoRegistration", excp);
                }
                finally
                {
        ...
        private void StopReg(bool sendZeroExpiryRegister = true)
        {
            try
            {
                if (!m_exit)
            ...
            {
                SQ.Base.ErrorLog.WriteLog4Ex("StopReg", excp);
        ...
        private void SendInitialRegister()
        {
            ...
            {
                SQ.Base.ErrorLog.WriteLog4Ex("SendInitialRegister", excp);
        ...
        private void ServerResponseReceived(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint, SIPTransaction sipTransaction, SIPResponse sipResponse)
        {
                ...
                if (sipResponse.Status == SIPResponseStatusCodesEnum.ProxyAuthenticationRequired || sipResponse.Status == SIPResponseStatusCodesEnum.Unauthorised)
                {
                    if (sipResponse.Header.HasAuthenticationHeader)
                    {
                        ...
                        else
                        {
                            ...
                            string server_id = (m_authUsername != null) ? m_authUsername : m_sipAccountAOR.User;
                            var authenticatedRequest = sipTransaction.TransactionRequest.DuplicateAndAuthenticate(
                                sipResponse.Header.AuthenticationHeaders, server_id, m_password);
            ...
            {
                SQ.Base.ErrorLog.WriteLog4Ex("ServerResponseReceived", excp);
        ...
        private void AuthResponseReceived(SIPEndPoint localSIPEndPoint, SIPEndPoint remoteEndPoint, SIPTransaction sipTransaction, SIPResponse sipResponse)
        {
            ...
            {
                SQ.Base.ErrorLog.WriteLog4Ex("AuthResponseReceived", excp);
        ...
        private SIPRequest GetRegistrationRequest()
        {
            ...
            toSIPToHeader = new SIPToHeader(this.UserDisplayName, m_sipAccountAOR, null);

            SIPURI fromURI = m_contactURI.CopyOf();
            fromURI.User = m_device_id;
            fromSIPFromHeader = new SIPFromHeader(this.UserDisplayName, fromURI, CallProperties.CreateNewTag());
